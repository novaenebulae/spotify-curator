...
COMPOSE_LAMBDA_PG := $(COMPOSE_LAMBDA_API) --profile postgres
...
lambda-up-a10-stable-tf6:
	$(COMPOSE_LAMBDA) up -d --scale audio-downloader=6 \
		--scale preview-resolver-worker=1 \
		--scale essentia-lowlevel-worker=3 \
		--scale essentia-tensorflow-worker=5 \
		--scale pipeline-ticker-worker=1

# A10 stable-tf6 + PostgreSQL sidecar (6 TF, no SQLite single-writer ceiling).
lambda-up-a10-stable-tf6-pg:
	$(COMPOSE_LAMBDA_PG) up -d postgres
	$(COMPOSE_LAMBDA_PG) run --rm core-api uv run alembic upgrade head
	$(COMPOSE_LAMBDA_PG) up -d --scale audio-downloader=6 \
		--scale preview-resolver-worker=1 \
		--scale essentia-lowlevel-worker=3 \
		--scale essentia-tensorflow-worker=6 \
		--scale pipeline-ticker-worker=1

lambda-migrate-sqlite-to-postgres:
